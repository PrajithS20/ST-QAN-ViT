...
def get_metrics(loader, model, device):
    ...
    return acc, sensitivity, specificity, np.mean(all_preds)
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        # Detailed Validation
        val_acc, val_sens, val_spec, pred_mean = get_metrics(val_loader, model, DEVICE)
        
        print(f"Epoch {epoch+1}: Loss: {avg_train_loss:.4f} | Val Acc: {val_acc:.4f} | SENSITIVITY: {val_sens:.4f} | Specificity: {val_spec:.4f}")
        print(f"  > Mean Prediction (should not be 0 or 1): {pred_mean:.4f}")

        # Save model if Sensitivity improves (We care about catching seizures!)
        if val_sens > best_sensitivity:
            ...
            print("  --> Best Sensitivity Model Saved!")
             ...
             # If sensitivity matches, prioritize accuracy
             torch.save(model.state_dict(), MODEL_SAVE_DIR / "best_model.pth")
